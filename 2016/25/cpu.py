...
class CPU:
    def __init__(self, prog, r, buflen=TEST_LEN):
    ...
    def done(self):
        return self.pc >= len(self.prog) or len(self.buf) >= self.buflen
...
def solve():
    for i in range(256):
        buf = run(overrides={'a': i})
        print i, buf
        if buf.startswith('01'*(TEST_LEN>>1)) or buf.startswith('10'*(TEST_LEN>>1)):
            print 'Answer:', i
            return
    print 'Not found'
