...
showL :: LSys -> String
showL (state, offset, _) = map (\x -> if x then '#' else '.') state
...
parse s = (parseState initial, 0, M.fromList $ map parseRule rest)
    where
        ...
        parseRule xs  = (parseState from, to == "#")
            where [from, to] = splitOn " => " xs

step :: LSys -> LSys
step (state, offset, rules)
    = (drop initialEmpty out, offset', rules)
    where
        state'       = divvy 5 1 (False:False:False:False:state++[False,False,False,False])
        out          = map (rules M.!) state'
        initialEmpty = length . head . group $ out
        offset'      = offset + initialEmpty - 2

potSum :: LSys -> Int
potSum (state, offset, _) = sum . map (\(n,x) -> if x then n else 0) $ (zip [offset..] state)
