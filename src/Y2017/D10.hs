...
day10a :: String -> Int
day10a input =
    let x:y:_ = rolls 256 . read $ '[' : input ++ "]"
     in x * y

day10b :: String -> String
day10b input = map (foldl' xor 0) dense >>= printf "%02x"
  where
    sparse =
        rolls 256 . concat . replicate 64 $
